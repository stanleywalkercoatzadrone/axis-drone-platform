...
export const updatePersonnel = async (req, res) => {
    try {
        ...
        const result = await db.query(
            ...
            swift_code = COALESCE($17, swift_code),
            secondary_phone = COALESCE($18, secondary_phone),
            emergency_contact_name = COALESCE($19, emergency_contact_name),
            emergency_contact_phone = COALESCE($20, emergency_contact_phone),
            tax_classification = COALESCE($21, tax_classification),
            city = COALESCE($22, city),
            state = COALESCE($23, state),
            zip_code = COALESCE($24, zip_code),
            country = COALESCE($25, country),
            account_type = COALESCE($26, account_type),
            updated_at = CURRENT_TIMESTAMP
            WHERE id = $27 AND (tenant_id = $28::text OR (tenant_id IS NULL AND $28 IS NULL))
            RETURNING * `,
            [
                fullName || null, role || null, email || null, phone || null, certificationLevel || null, payRate || null, travelDist || null, status || null,
...
export const uploadPersonnelDocument = async (req, res) => {
    try {
        ...
        const docResult = await db.query(
            `INSERT INTO pilot_documents(pilot_id, country_id, document_type, file_url, validation_status, expiration_date)
            VALUES($1, $2, $3, $4, 'PENDING', $5)
            RETURNING * `,
            [id, countryId || null, documentType, webViewLink, expirationDate || null]
        );
...
/**
 * Analyze personnel document (No DB Save)
 */
export const analyzePersonnelDocument = async (req, res) => {
    try {
        const file = req.file;
        console.log(`📡 [DEBUG] analyzePersonnelDocument called - File: ${file?.originalname}, Mimetype: ${file?.mimetype}`);
        fs.appendFileSync('ai_debug.log', `[${new Date().toISOString()}] analyzePersonnelDocument CALLED - File: ${file?.originalname}\n`);
        ...
        const prompt = `
            Analyze this document (image or PDF). Extract professional and banking information for a personnel record.
            ...
            - documentType: Must be one of ["Direct Deposit", "W9", "Driver License", "FAA License", "Insurance", "Passport", "Other"]
        ...
        // Debug logging to file (Relative path for container compatibility)
        const logPath = 'ai_debug.log';
        fs.appendFileSync(logPath, `[${new Date().toISOString()}] analyzePersonnelDocument RAW RESPONSE: ${text}\n`);
        ...
        console.log('🤖 AI Analysis Result:', extractedData);
        fs.appendFileSync('ai_debug.log', `[${new Date().toISOString()}] PARSED DATA: ${JSON.stringify(extractedData)}\n`);
