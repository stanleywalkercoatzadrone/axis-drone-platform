...
import { Severity, Industry } from '../../../types';
...
const ReportReview: React.FC<ReportReviewProps> = ({ onBack }) => {
    ...
    const generateModularPDF = async () => {
        const totalCost = images.flatMap(img => img.annotations).reduce((sum, ann: any) => sum + (ann.estimatedCostMin || 0), 0);
        
        if (industry === Industry.SOLAR) {
            const solarFindings = images.flatMap(img => img.annotations.map((a: any, i) => ({
                id: a.id,
                type: a.label,
