...
import { X, File as FileIcon, Folder, AlertCircle, CheckCircle, RotateCw, Briefcase, MapPin } from 'lucide-react';
import apiClient from '../../services/apiClient';
import { updateCompliance } from '../../utils/compliance';
...
export const UploadCenter: React.FC = () => {
    ...
    React.useEffect(() => {
        if (mission.site && mission.client) {
            // Fetch site name for context
            apiClient.get('/assets/sites', { params: { clientId: mission.client } })
                .then(res => {
                    const site = res.data.data?.find((s: any) => s.id === mission.site);
    ...
    const startUpload = async () => {
        if (!mission.client || !mission.site) {
        ...
        try {
            // 1. Create Job
            const jobRes = await apiClient.post('/uploads/jobs', {
                industry: mission.industry,
                clientId: mission.client,
                siteId: mission.site,
            ...
            if (jobRes.data.success) {
                ...
                await apiClient.post(`/uploads/jobs/${newJobId}/files`, { files: filesMeta });
        ...
        const interval = setInterval(() => {
            setQueue(prev => {
                ...
                if (next.every(f => f.status === 'completed')) {
                    ...
                    setIsUploading(false);
                    // Trigger compliance automation if user is present
                    if (user?.id) {
                        updateCompliance(user.id);
                    }
                }
                return next;
            });
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <header className="flex items-center justify-between">
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        ...
                        {mission.client && (
                            <span className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 text-[10px] font-bold uppercase tracking-wider border border-blue-500/20">
                                <Briefcase className="w-3 h-3" />
                                {mission.client}
                            </span>
                        )}
                        {siteName && (
                            <span className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 text-[10px] font-bold uppercase tracking-wider border border-emerald-500/20">
            ...
            {queue.length === 0 ? (
                <FileDropZone onFilesSelected={handleFilesSelected} disabled={!mission.site || isUploading} />
            ) : (
