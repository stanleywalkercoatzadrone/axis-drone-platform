...
import { Globe, Building, MapPin, Briefcase, ChevronDown, TrendingUp } from "lucide-react";
        ...
        <div className="flex items-center gap-4 bg-slate-950/50 p-1 rounded-xl border border-slate-800 backdrop-blur-sm">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors cursor-pointer group">
                ...
                >
                    ...
                    {/* Dynamically populated in future steps */}
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors cursor-pointer group">
                ...
                <select
                    value={mission.industry || ""}
                    onChange={(e) => handleSelect("industry", e.target.value)}
                    className="bg-transparent text-xs font-bold uppercase tracking-wider text-slate-300 outline-none cursor-pointer appearance-none pr-6"
                >
                    <option value="" className="bg-slate-900">Industry</option>
                    <option value="Solar" className="bg-slate-900">Solar</option>
                    <option value="Telecom" className="bg-slate-900">Telecom</option>
                ...
                <ChevronDown className="w-3 h-3 text-slate-500 -ml-5 group-hover:text-amber-400 transition-colors pointer-events-none" />
            </div>
        </div>
    );
}
