...
export const syncToVault = async (req, res, next) => {
    try {
        ...
        // Verify report ownership and get user Drive credentials
        const result = await query(
            `SELECT r.*, u.drive_linked, u.drive_access_token, u.drive_refresh_token, u.drive_folder
       FROM reports r
       ...
       WHERE r.id = $1 AND r.user_id = $2`,
        ...
        if (!report.drive_linked || !report.drive_access_token) {
        ...
        try {
            ...
            const userFolder = await findOrCreateFolder(
                report.drive_access_token,
                report.drive_refresh_token,
                userFolderName
            );
