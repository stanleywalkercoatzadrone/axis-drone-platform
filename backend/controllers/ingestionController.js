...
/**
 * createJob - Starts a new ingestion batch
...
export const createJob = async (req, res) => {
    ...
    try {
        ...
        // Log creation event
        await query(
            `INSERT INTO ingestion_events (job_id, level, message, meta) VALUES ($1, 'info', 'Ingestion job created', $2)`,
            [job.id, JSON.stringify({ user: userId, totalFiles })]
        );

        res.status(201).json({ success: true, data: job });
    } catch (err) {
        console.error('Error creating ingestion job:', err);
 ...
 * POST /api/ingestion/jobs/:id/exceptions
 */
    ...
    try {
        await transaction(async (client) => {
            ...
            await client.query(
                ...
                VALUES ($1, $2, 'todo', $3, 'ingestion_exception', $4, $5, $6)`,
                [
                    `Ingestion Exception: ${type}`,
